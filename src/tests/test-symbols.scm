...
(add-tests-with-string-output "symbols linked list"
 ...
 [(prim-apply null? (prim-apply car symbols-list)) => "#t\n"]
 [(prim-apply null? (prim-apply cdr symbols-list)) => "#t\n"]

 [(let* ((s1 (string->symbol "foobar"))
         (ls (prim-apply car symbols-list)))
    (string=? "foobar" (prim-apply symbol->string (prim-apply car ls))))
    => "#t\n"]

 [(let* ((s1 (string->symbol "foobar"))
         (s2 (string->symbol "foobar")))
    (prim-apply car symbols-list)) => "(foobar)\n"])
